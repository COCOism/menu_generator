...
# 定義菜單生成邏輯
def generate_menu_for_course(course):
    menu_pool = {
        "主食": [
    ...
    return random.choice(menu_pool[course])
...
def main():
    ...
    if "menu" not in st.session_state:
        st.session_state["menu"] = {
            "主食": generate_menu_for_course("主食"),
            "主菜": generate_menu_for_course("主菜"),
            "副菜": generate_menu_for_course("副菜"),
            "湯品": generate_menu_for_course("湯品"),
        }
    ...
    for course in ["主食", "主菜", "副菜", "湯品"]:
        ...
        with col2:
            if st.button(f"重新生成 {course}", key=course):
                st.session_state["menu"][course] = generate_menu_for_course(course)
